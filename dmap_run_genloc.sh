...
#
# Peter Stockwell: Oct,Nov-2021, Jun-2023.
#  add input line length check Nov-2025, 29-Jan-2026
...
printf "Checking for input column count\n";
...
# check for max line length of input file: Note: requires identgeneloc V0.30 or later.

printf "Checking for maximum input line length for '${genloc_input_file}'\n";

inbuflen=`awk 'BEGIN{maxlen=0;}{if(length($0)>maxlen)maxlen=length($0);}END{print maxlen;}' ${genloc_input_file}`;
...
printf "feature_annotation_type ${feature_annotation_type}\n"
...
;;
  GFF3)
    if [[ "${annotation_files[0]##*.}" == "gz" ]]; then
      ...
      gtfgff3buflen=`gzip -dc "${annotation_file_location}""${annotation_files[@]%.*}" |awk 'BEGIN{maxlen=0;}{if(length($0)>maxlen)maxlen=length($0);}END{print maxlen;}'`
    else
      add_opt_and_value "-F -f" "${annotation_file_location}""${annotation_files[@]}";
      gtfgff3buflen=`awk 'BEGIN{maxlen=0;}{if(length($0)>maxlen)maxlen=length($0);}END{print maxlen;}' "${annotation_file_location}""${annotation_files[@]}"`;
    fi
;;
  GTF)
    if [[ "${annotation_files[0]##*.}" == "gz" ]]; then
...
# put this option here to allow for GTF and GFF3 input line lengths which also use the srcbuflen parameter

if [[ ${inbuflen} -gt ${gtfgff3buflen} ]]; then

  add_opt_and_value "-b" "${inbuflen}";

else

  add_opt_and_value "-b" "${gtfgff3buflen}";
...
case ${feature_annotation_type} in
  ...
  SeqMonk)
    ...
    if [[ ${#genloc_biotypes[@]} > 0 ]]; then
      for biotype in "${genloc_biotypes[@]}"; do
